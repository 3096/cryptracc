...
export default function FileHashing() {
    const [algorithms] = useState(['sha256', 'sha384', 'sha512']);
    ...
    const handleFileInput = (e : any) => {
        ...
        fr.onload = async () => {
            ...
            if (algorithm == 'sha256') {
                ...
                result = await sha384(fr.result as string);
            } else if (algorithm == 'sha512') {
                result = await sha512(fr.result as string);
            }
    ...
    const handleAlgorithmChange = async (e : any) => {
